This project is generated by [deepbolt](https://deepbolt.xyz/) with prompt:

```
构建一个app能够播放本地mp3文件，并且支持转调，用户可以选择不同的转调。
界面支持中文和英文，根据用户环境自动选择。
转调支持 12 个半音音阶。
提示：可以使用AudioBufferSourceNode 接口的detune 属性改变播放音乐的 pitch。
```
